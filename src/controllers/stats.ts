...
const orderMonthlyRevenue=({length:6,today,docArr:lastSixMonthOrders,property:"total"})
...
const modifiedLatestTransaction=latestTransaction.map(
    (i)=>({
        _id:i._id,
        discount:i.discount,
        amount:i.total,
...
const [processingOrder,shippedOrder,deliveredOrder,categories,
    productCount,outOfStock,allOrders,
    allUsers,
    ...
    Product.distinct("category"),
    Product.countDocuments(),
    Product.countDocuments({stock:0}),allOrderPromise,
...
charts={
    ...
    stockAvailablity,
    userAgeGroup,
    revenueDistribution,adminCustomer
}
...
export const getBarCharts=TryCatch(async(req,res,next)=>{
 ...
 if(myCache.has(key)) charts=JSON.parse(myCache.get(key) as string);
        ...
        const [products,users,orders]=await Promise.all([sixMonthProductPromise,
